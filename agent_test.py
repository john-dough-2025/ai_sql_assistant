...
from agents_utils import Agent, Runner, trace, Tool, function_tool
...
manager_agent = Agent(
    ...
    instructions=(
        ...
        "1) generate_sql 2) query_snowflake 3) create_chart 4) validate."
...
#     name="run_code_agent",
#     instructions=(
#         "You are in charge of executing the code provided "
#     ),
#     tools=[
#         sql_agent.as_tool(tool_name="generate_sql", tool_description="Generate Snowflake SQL"),
#         query_snowflake,
...
# 6. Example orchestration entrypoint
async def main():
    user_request = input("Describe your data request: ")
    with trace("Snowflake-Streamlit Orchestration"):

        result = await Runner.run(manager_agent, user_request)
    print("\n--- Final Output ---\n")
